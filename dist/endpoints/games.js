...
class Games {
    ...
    get(userParam, options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!Object.keys(options).every(k => k in def))
                throw new Error('Query parameter doesn\'t exist!');
            if (!Object.values(options).every(v => typeof v === 'boolean'))
                throw new Error('Query parameter must take a boolean value!');
            options = Object.assign(def, options);
            if (typeof userParam === 'string')
                return this.getOne(userParam, options);
            if (Array.isArray(userParam)) {
                if (userParam.length === 1)
                    return this.getOne(userParam[0], options);
    ...
    getOne(id, options) {
        return __awaiter(this, void 0, void 0, function* () {
            if (typeof id !== 'string' || !/[a-z][\w-]{0,28}[a-z0-9]/i.test(id))
                throw new TypeError('game ID for lichess.games.export() must be a string');
            if (id.length > 8)
                id = id.slice(0, 8);
            try {
                let result = new Game(JSON.parse(yield rp.get({
                ...
                let users = yield this.Users.getMultiple(result.players.keyArray());
    ...
    getMultiple(ids, options) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!Array.isArray(ids))
                throw new TypeError('lichess.users.getMultiple() takes an array as an input');
            if (!ids.every(n => typeof n === 'string' && /[a-z][\w-]{0,28}[a-z0-9]/i.test(n)))
     ...
     */
    byUser(username, options = {}, stream = false, filepath) {
        let keys = ['since', 'until', 'max', 'vs', 'perfType', 'color', 'rated', 'analysed', 'ongoing', 'moves', 'tags', 'clocks', 'evals', 'opening'];
        if (typeof username !== 'string')
            throw new TypeError('game ID for lichess.games.exportUser() must be a string');
        if (!/[a-z][\w-]{0,28}[a-z0-9]/i.test(username))
            throw new TypeError('Invalid format for lichess username: ' + username);
        if (options.since && options.since < 1356998400070)
            throw new Error('Date specified is before date of Lichess site launch');
        if (options.until && options.until < 1356998400070)
            throw new Error('Date specified is before date of Lichess site launch');
        if (typeof options.max !== 'undefined' && options.max !== null && (typeof options.max !== 'number' && options.max < 1))
            throw new Error('Could not interpret value for max number of games to download');
        if (options.vs && typeof options.vs !== 'string')
            throw new TypeError('game ID for lichess.games.export() must be a string');
        if (options.vs && !/[a-z][\w-]{0,28}[a-z0-9]/i.test(options.vs))
            throw new TypeError('Invalid format for lichess username: ' + options.vs);
        if (options.perfType) {
            if (typeof options.perfType === 'string')
                options.perfType = [options.perfType];
            else if (!Array.isArray(options.perfType))
                throw new TypeError('perfType parameter must be a variant represented as a string or an array containing variants from the specific lichess list: ' + config.variants.join(','));
            for (let variant of options.perfType) {
                if (config.variants.indexOf(variant.trim()) === -1)
                    throw new TypeError('variants specified must be from the specific list of lichess variant keys, separated by a comma: ' + config.variants.join(','));
            }
            options.perfType = options.perfType.join(',');
        }
        if (typeof options.color !== 'undefined' && typeof options.color !== 'string' && !/^(?:white|black)$/.test(options.color))
            throw new TypeError('color parameter takes a string value of \'white\' or \'black\'');
        for (let option of ['rated', 'analysed', 'ongoing', 'moves', 'tags', 'clocks', 'evals', 'opening']) {
            if (typeof options[option] !== 'undefined' && typeof options[option] !== 'boolean')
                throw new TypeError(option + ' parameter takes a boolean value');
        }
            ...
            switch (Boolean(stream)) {
                ...
                case false:
                    return Promise.resolve((() => __awaiter(this, void 0, void 0, function* () {
                        new GameStore(Util.ndjson(yield rp.get(req)));
                    }))());
                default:
     ...
     */
    current(usernames, options) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!Array.isArray(usernames))
                throw new TypeError('lichess.games.current() takes an array as an input');
            if (!usernames.every(n => typeof n === 'string' && /[a-z][\w-]{0,28}[a-z0-9]/i.test(n)))
                throw new SyntaxError('Invalid format for lichess username.');
            try {
                let options = {
                    method: 'POST',
    ...
    ongoing(nb = 9) {
        return __awaiter(this, void 0, void 0, function* () {
            if (typeof nb !== 'number')
                throw new TypeError('Number of games to fetch must be type Number');
            if (nb < 1 || nb > 50)
                throw new TypeError('nb value provided is out of range 1-50: ' + nb);
            try {
                return yield rp.get({
                    method: 'GET',
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (variants) {
                ...
                if (!Array.isArray(variants))
                    throw new TypeError('variants parameter must be a variant represented as a string or an array containing variants from the specific lichess list: ' + config.variants.join(','));
                if (!variants.every(v => config.variants.indexOf(v.trim())))
                    throw new TypeError('variants specified must be from the specific list of lichess variant keys, separated by a comma: ' + config.variants.join(','));
            }
            try {
                let data = yield rp.get({
